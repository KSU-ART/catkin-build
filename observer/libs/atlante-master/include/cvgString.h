...
/* Since constructors for all basic types are defined, there is no need to define 
	...
	cvgString ltrim(const cvgString &s = " ") const;
	cvgString rtrim(const cvgString &s = " ") const;
	cvgString trim(const cvgString &s = " ") const;
	...
	cvg_long toLong(cvg_bool *result = NULL);
