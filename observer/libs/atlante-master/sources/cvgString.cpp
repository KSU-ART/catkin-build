...
cvgStringVector cvgString::split(const cvgString &token, size_t offset) const {
	...
	while(loop) {
		...
		if (offset > pieceStart || !loop || (offset == 0 && pieceStart == 0))
...
cvg_long cvgString::toLong(cvg_bool *result) {
	cvg_long value;
	if (sscanf(c_str(), "%"CVG_PRINTF_PREFIX_LONG"d", &value) == 1) {
		if (result != NULL) (*result) = true;
		return value;
	} else {
		if (result == NULL)
			throw cvgException("[cvgString] Cannot convert string to long");
		else (*result) = false;
		return 0;		
	} 
}
...
cvgString cvgString::ltrim(const cvgString &s) const {
	size_t offset = 0, pos;
	do {
		pos = find(s, offset);
		if (pos != cvgString::npos && pos == offset) offset += s.length();
		else break;
	} while(true);
	return subString(offset);
}

cvgString cvgString::rtrim(const cvgString &s) const {
	size_t offset = length() - 1, pos;
	do {
		pos = rfind(s, offset);
		if (pos != cvgString::npos && pos == offset) offset -= s.length();
		else break;
	} while(true);
	return subString(0, offset + 1);
}

cvgString cvgString::trim(const cvgString &s) const {
